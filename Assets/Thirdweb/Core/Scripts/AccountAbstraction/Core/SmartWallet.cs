    ...
    public class SmartWallet
    {
        ...
        public SmartWallet(Web3 personalWeb3, ThirdwebSDK.SmartWalletConfig config)
        {
            ...
            Config = new ThirdwebSDK.SmartWalletConfig()
            {
                ...
                bundlerUrl = string.IsNullOrEmpty(config.bundlerUrl)
                    ? $"https://{ThirdwebManager.Instance.SDK.session.CurrentChainData.chainName}.bundler.thirdweb.com"
                    : config.bundlerUrl,
                paymasterUrl = string.IsNullOrEmpty(config.paymasterUrl)
                    ? $"https://{ThirdwebManager.Instance.SDK.session.CurrentChainData.chainName}.bundler.thirdweb.com"
                    : config.paymasterUrl,
                entryPointAddress = string.IsNullOrEmpty(config.entryPointAddress)
        ...
        internal async Task Initialize(string smartWalletOverride = null)
        {
            ...
            var predictedAccount =
                ...
                ?? (
                    await TransactionManager.ThirdwebRead<
                        FactoryContract.GetAddressFunction,
                        FactoryContract.GetAddressOutputDTO
                    >(
                        Config.factoryAddress,
                        new FactoryContract.GetAddressFunction()
                        {
                            AdminSigner = PersonalAddress,
                            Data = new byte[] { }
                        }
                    )
                ).ReturnValue1;
            ...
            ThirdwebDebug.Log(
                $"Initialized with Factory: {Config.factoryAddress}, AdminSigner: {PersonalAddress}, Predicted Account: {Accounts[0]}, Deployed: {_deployed}"
            );
        }
        ...
        internal async Task<TransactionResult> SetPermissionsForSigner(
        ...
        )
        {
            return await TransactionManager.ThirdwebWrite(
                Accounts[0],
                new SetPermissionsForSignerFunction()
                {
        ...
        {
            var verifyRes = await TransactionManager.ThirdwebRead<
                AccountContract.IsValidSignatureFunction,
                AccountContract.IsValidSignatureOutputDTO
            >(
                Accounts[0],
                new AccountContract.IsValidSignatureFunction()
                {
                    Hash = hash,
                    Signature = signature
                }
            );
            return verifyRes.MagicValue.ToHex(true)
                == new byte[] { 0x16, 0x26, 0xba, 0x7e }.ToHex(true);
        }
        ...
        internal async Task<(byte[] initCode, BigInteger gas)> GetInitCode()
        {
            ...
            var fn = new FactoryContract.CreateAccountFunction()
            {
                Admin = PersonalAddress,
                Data = new byte[] { }
            };
            var deployHandler = Utils
                .GetWeb3()
                .Eth.GetContractTransactionHandler<FactoryContract.CreateAccountFunction>();
            var txInput = await deployHandler.CreateTransactionInputEstimatingGasAsync(
                Config.factoryAddress,
                fn
            );
        ...
        internal async Task<RpcResponseMessage> Request(RpcRequestMessage requestMessage)
        {
            ...
            else if (requestMessage.Method == "eth_chainId")
            {
                ...
                catch
                {
                    return new RpcResponseMessage(
                        requestMessage.Id,
                        ThirdwebManager.Instance.SDK.session.CurrentChainData.chainId
                    );
                }
            }
            else if (requestMessage.Method == "eth_estimateGas")
            {
                ...
                var parameters = JsonConvert.DeserializeObject<object[]>(
                    JsonConvert.SerializeObject(requestMessage.RawParameters)
                );
                var txInput = JsonConvert.DeserializeObject<TransactionInput>(
                    JsonConvert.SerializeObject(parameters[0])
                );
        ...
        private async Task<RpcResponseMessage> CreateUserOpAndSend(RpcRequestMessage requestMessage)
        {
            ...
            var paramList = JsonConvert.DeserializeObject<List<object>>(
                JsonConvert.SerializeObject(requestMessage.RawParameters)
            );
            var transactionInput = JsonConvert.DeserializeObject<TransactionInput>(
                JsonConvert.SerializeObject(paramList[0])
            );
            ...
            var gasPrices = await Utils.GetGasPriceAsync(
            ...
            partialUserOp.PaymasterAndData = await GetPaymasterAndData(
                requestMessage.Id,
            ...
            partialUserOp.VerificationGasLimit = new HexBigInteger(
                gasEstimates.VerificationGas
            ).Value;
            partialUserOp.PreVerificationGas = new HexBigInteger(
            ...
            partialUserOp.Signature = await partialUserOp.HashAndSignUserOp(
                Config.entryPointAddress
            );
            ...
            ThirdwebDebug.Log(
                "Valid Encoded UserOp: "
                    + JsonConvert.SerializeObject(partialUserOp.EncodeUserOperation())
            );
            ...
            while (txHash == null)
            {
                var getUserOpResponse = await BundlerClient.EthGetUserOperationByHash(
                    Config.bundlerUrl,
                    apiKey,
                    requestMessage.Id,
                    userOpHash
            ...
            if (!_deployed)
            {
                ...
                var decodedEvents =
                    receipt.DecodeAllEvents<EntryPointContract.UserOperationEventEventDTO>();
        ...
        {
            var nonce = await TransactionManager.ThirdwebRead<
                EntryPointContract.GetNonceFunction,
                EntryPointContract.GetNonceOutputDTO
            >(
                Config.entryPointAddress,
                new EntryPointContract.GetNonceFunction()
                {
                    Sender = Accounts[0],
                    Key = UserOpUtils.GetRandomInt192()
                }
            );
            return nonce.Nonce;
        }

        private async Task<byte[]> GetPaymasterAndData(
            object requestId,
        ...
        )
        {
            return Config.gasless
                ? (
                    await BundlerClient.PMSponsorUserOperation(
                        Config.paymasterUrl,
                        apiKey,
                        requestId,
                        userOp,
                ...
                ).paymasterAndData.HexStringToByteArray()
                : new byte[] { };
        }
    }
}
